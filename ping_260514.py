...
def render_attendance_tab(df, col_date, is_admin, live_names):
    ...
    # 🎨 [UI/디자인 수정] 출석체크 표를 좌우 1:1 비율로 나눕니다. [1, 2]로 바꾸면 오른쪽 표가 2배 넓어집니다.
...
def render_draw_process(cfg, sorted_members, total_levels, draw_level):
    ...
    # 🌟 1. 결과를 메인 데이터프레임에 반영하는 내부 함수 (최상단에 한 번만 정의)
    ...
    # 🌟 2. 완료 체크 로직 (화면 렌더링 전에 미리 체크하여 불필요한 렌더링 방지)
    # ✅ [수정 후] 키가 없으면 기본값 0을 반환하여 에러 방지
    if st.session_state.get('draw_level', 0) >= total_levels:
    # if st.session_state.draw_level >= total_levels:
        if not st.session_state.get('draw_completed', False):
            ...
            st.rerun()
        return # 완료되었으면 아래 UI를 그리지 않고 함수 종료

    # ---------------------------------------------------------
    # 🚀 3. UI 렌더링 부분 (중복 제거된 단일 반복문)
    # ---------------------------------------------------------
    for level in range(total_levels):
        ...
        # 1️⃣ 이미 완료된 그룹 (간략하게 한 줄로 표시하여 공간/속도 절약)
        if level < draw_level:
            result_texts = []
            ...
            st.markdown(
                f"<div style='color:#155724; background-color:#d4edda; padding:8px; border-radius:5px; margin-bottom:2px; font-size:14px;'>"
        ...
        # 2️⃣ 현재 진행 중인 그룹
            ...
            st.divider()
            responsive_text(f"🎯 그룹 {level + 1} 진행 중", pc_size="20px", mobile_size="16px")
            ...
            # 세션 데이터 초기화
            if state_key not in st.session_state or not all(k in st.session_state[state_key] for k in unique_keys):
                st.session_state[state_key] = {u_key: available_options[i] for i, u_key in enumerate(unique_keys)}
                st.session_state[f"{state_key}_prev"] = st.session_state[state_key].copy()

            # 🌟 [수정됨] 변수명을 u_key로 통일하고 .get()을 사용하여 에러 원천 차단
            def on_selection_change(u_key, lvl):
                s_key = f"group_selections_{lvl}"
                prev_selections = st.session_state.get(f"{s_key}_prev", {})
                current_selections = st.session_state.get(s_key, {})

                new_val = st.session_state.get(f"select_{lvl}_{u_key}")
                ...
                if new_val is not None and old_val is not None and new_val != old_val:
                    for other_key, val in prev_selections.items():
                        if other_key != u_key and val == new_val:
                            ...
                            current_selections[other_key] = old_val
                            break
                    current_selections[u_key] = new_val
            ...
            for i, (idx, row) in enumerate(current_group.iterrows()):
                ...
                with cols[i % cfg['g']]:
                    st.markdown(f" **{name}**")
                    current_val = st.session_state[state_key].get(u_key, available_options[0])

                    st.selectbox("조 선택", options=available_options, index=available_options.index(current_val),
                                 key=f"select_{level}_{u_key}", on_change=on_selection_change,
                                 args=(u_key, level), label_visibility="collapsed",
        ...
        # 3️⃣ 대기 중인 그룹
            ...
            st.markdown(
                f"<div style='color:#6c757d; background-color:#f8f9fa; padding:8px; border-radius:5px; margin-bottom:5px; font-size:14px;'>"
...
# ---------------------------------------------------------------------
    ...
    # ==========================================
    with tab_config:
        # 💡 [간격 조절] 탭 메뉴와 참석 인원 정보창 사이의 상단 여백 (줄이려면 숫자를 줄이거나 음수 적용 불가, 0이 최소)
        v_space(5)
        ...
        #     with st.expander("👀 참석자 상세 명단 보기"):
        #         st.write(live_names)

        # 🌟 [수정] 빈 Box나 기본 st.divider() 대신, 위아래 여백이 없는 얇은 선을 직접 그립니다.
        v_space(5)
        ...
        if st.session_state.get("config_confirmed", False):
            # st.success(" 운영 설정 및 조 편성이 확정되었습니다. ")
            ...
            # 🌟 요약 정보를 Expander로 감싸서 공간 절약
            # with st.expander("📌 현재 확정된 설정 요약 보기", expanded=False):
            #     st.markdown(f"""
            #         * 편성 조 수: {saved_cfg.get('g')}조 _ <strong> (조당 {per_group_text}) </strong>
            ...
            # [관리자 전용] 설정 잠금 해제 버튼
            # if is_admin:
            #     st.warning("⚠️ 설정을 변경하면 진행 중인 조 편성과 대진표가 모두 초기화됩니다.")
            #     if st.button("🔓 설정 하기", type="secondary"):
            #         # 🌟 [적용 1] 길었던 초기화 코드를 함수 한 줄로 대체!
            #         reset_draw_state(reset_config=True)
            #         st.rerun()

            # st.columns를 사용해 화면을 3칸으로 나눔 (비율 2 : 4 : 1)

            col_btn, col_info = st.columns([1, 3], gap="small")
            ...
            with col_info:
                if is_admin:
                    st.warning(" 설정을 변경하면 진행 중인 조 편성과 대진표가 모두 초기화됩니다.")
                    # 🌟 [디자인 적용] 연한 녹색 바탕, 수직 중앙 정렬(일직선 배치)
                    # st.markdown("""
                    #                <div style='
                    #                    background-color: #e8f5e9; /* 연한 녹색 바탕 */
                    #                    color: #2e7d32; /* 진한 녹색 글씨 (가독성) */
                    #                    border: 1px solid #c8e6c9; /* 테두리 살짝 */
                    #                    border-radius: 8px; /* 둥근 모서리 */
                    #                    height: 40px; /* Streamlit 버튼 기본 높이와 맞춤 */
                    #                    display: flex;
        ...
        # 설정이 확정되지 않은 경우 (입력 화면)
            ...
            if is_admin:
                # [관리자 전용] 설정 입력 폼
                # 💡 [상하 간격 조절] -20px 숫자를 -30px, -40px 등으로 키울수록 위쪽 선과 더 가까워집니다.
                st.markdown("<div style='margin-top: -20px;'></div>", unsafe_allow_html=True)
                # 🎨 [UI/디자인 수정] 운영 설정 탭의 5개 항목 너비 비율 조정
                c1, c2, c3, c4, c5 = st.columns([1.2, 1.2, 1.2, 1.2, 1.5], gap="small")

                with c1:
                    responsive_text(f"👥 조 구성", pc_size="20px", mobile_size="16px")
                    v_space(5)  # 💡 [간격 조절] 제목과 입력창 사이
                    ...
                    safe_default_g = min(default_g, max_groups)  # 기본값도 max_groups를 넘지 않도록 조정

                    g_val = st.number_input("편성 조 수", min_value=1, max_value=max_groups, value=safe_default_g, disabled=not is_admin)
                    if g_val > 0:
                ...
                with c5:
                    ...
                    if st.button("설정 확정 및 편성 시작", type="primary", use_container_width=True, disabled=is_empty_attendees):
                        ...
                        save_room_state(st.session_state.room_name)
                        st.toast("✅ 설정이 성공적으로 저장되었습니다!", icon="🎉")  # 알림 추가
                        time.sleep(0.5)  # 사용자가 알림을 볼 수 있도록 0.5초 대기 후 리런
                        st.rerun()
        ...
        if "config" in st.session_state and st.session_state.config.get('draw_method') == '제비뽑기':
            if not st.session_state.get('draw_completed', False):
                ...
                # # 🚨 [수정 1] 이전 대화에서 수정한 'attendees_df' 재사용 (데이터 불일치 버그 방지)
                attendees = attendees_df.copy()
                ...
                if attendees.empty:
                    ...
                    v_space(10)  # 💡 [간격 조절] 타이틀과 실제 제비뽑기 내용 사이
                ...
                st.success("✅ 제비뽑기가 모두 완료되었습니다! 상단의 설정 확정을 눌러주세요.")
    ...
    if tab_match is not None:
        with tab_match:
            ...
            if 'labels' in st.session_state and st.session_state.get('matrix') is not None:
                ...
                # 🎨 [UI/디자인 수정] 경기 배정표 상단 제목과 안내문구의 좌우 비율 (3:7)
    ...
    with tab_score:
        ...
        if 'labels' in st.session_state and st.session_state.get('matrix') is not None:
            ...
            # # 🎨 [UI/디자인 수정] 스코어보드 상단 제어 버튼들의 좌우 비율 조정
            ...
            if st.session_state.fullscreen_table:
                ...
                with col_title:
                    responsive_text(f"🏆 종합 결과 전광판", pc_size="22px", mobile_size="16px")
                with col_btn:
                ...
                def render_manual_score_input():
                    ...
                    if labels:
                        ...
                        c1, c2, c3, c4, c5 = st.columns([2, 2, 1.5, 2.5, 1.5], gap="medium")
                ...
                col_title, col_fs, col_dl1, col_dl2 = st.columns([4, 2, 2, 2])
                ...
                with col_fs:
                    if st.button("📺 전체 화면 모드", type="primary", use_container_width=True):
