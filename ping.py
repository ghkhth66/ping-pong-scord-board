...
def update_cumulative_record(p_a, p_b, s_a, s_b):
    ...
    if p_a != "선택안함" and p_b != "선택안함":
        ...
        p1, p2 = sorted([p_a, p_b])  # 항상 이름순으로 정렬하여 중복 방지
...
# ==========================================
# 💡 [상세 설명]
# Streamlit은 버튼을 누를 때마다 코드를 맨 위부터 다시 읽습니다.
...
if 'is_admin' not in st.session_state:
    st.session_state.is_admin = False  # 처음엔 관리자가 아님
if 'room_name' not in st.session_state:
    st.session_state.room_name = "생활_탁구장"  # 기본 방 이름
...
# ==========================================
# 구글 시트를 데이터베이스처럼 사용하여 구장 정보와 비밀번호를 저장/조회함
        ...
        if st.button("로그인 / 접속", use_container_width=True):
            ...
            if admin_password:
                ...
                if hashed_pw == HASHED_MASTER_PW:
                    ...
                    time.sleep(1)
                    st.rerun()  # 💡 로그인 성공 시 즉시 새로고침하여 사이드바를 바꿈!
        ...
        if st.button("새 구장 생성하기", type="primary", use_container_width=True):
            if not new_room_name or not admin_name or not admin_email or not new_room_pw:
                ...
                try:
                    ...
                    st.rerun()  # 화면을 새로고침하여 즉시 메인 화면(관리자 모드)으로 진입!
                    # ==========================================

                    # st.success(f"✅ '{new_room_name}' 구장이 생성되었습니다! 로그인해주세요.")
                except Exception as e:
                    st.error(f"DB 저장 중 오류가 발생했습니다: {e}")

# ---------------------------------------------------------
# 🟢 [로그인 후] 사이드바 화면 (탭이 사라지고 접속 정보만 보임)
# ---------------------------------------------------------
else:
    ...
    if st.sidebar.button("🔒 로그아웃", use_container_width=True):
        ...
        st.rerun()  # 로그아웃 시 다시 로그인 전 화면으로 돌아감
...
with tab_home:
    ...
    # 명단을 두 줄(좌/우)로 나누어 보여주기 위한 인덱스 계산
    mid_idx = len(df) // 2 + (len(df) % 2)
    col1, col2 = st.columns(2)

    # st.data_editor를 사용해 화면에서 직접 체크박스를 클릭해 출석 수정 가능
    with col1:
...
with tab_config:
    ...
    # 조 구성 설정 (나머지가 0일 때와 아닐 때 문구 다르게 출력)
    with c1:
        st.markdown(f"#### 👥 조 구성")
    ...
    # 제비뽑기 로직 (수동으로 조를 선택하거나 랜덤 배정하는 기능)
        ...
        if not st.session_state.get('draw_completed', False):
            ...
            for level in range(total_levels):
                ...
                # 현재 뽑기를 진행 중인 그룹
                    ...
                    # 중복 선택 방지 로직 (한 명이 1조를 고르면 다른 사람의 1조 선택 해제)
                    ...
                    for i, (idx, row) in enumerate(current_group.iterrows()):
                        ...
                        with cols[i % cfg['g']]:
                            ...
                            st.selectbox("조 선택", options=available_options, index=available_options.index(current_val),
                                         ...
                                         args=(name, level),
                                         label_visibility="collapsed", disabled=not is_admin)
                    st.write("")
                    if is_admin:
...
with tab_score:
    if 'labels' in st.session_state and st.session_state.get('matrix') is not None:
        ...
        # 상단 컨트롤 패널 (전체화면, 글자크기 조절, 다운로드 버튼)
        ...
        with col_ctrl2:
            ...
            with f_col1:
                if st.button("➖", help="글자 축소"): st.session_state.table_font_size = max(8,
            ...
            with f_col2:
                st.markdown(
                    ...
                    unsafe_allow_html=True)
            with f_col3:
                if st.button("➕", help="글자 확대"): st.session_state.table_font_size = min(30,
                                                                                        st.session_state.table_font_size + 1); st.rerun()
        with col_ctrl3:
        ...
        if is_fullscreen:
            ...
            if not is_ind:
                    ...
                    if len(players) > 0:
                        ...
                        with c2:
                            def format_player_b(p_b_name):
                                ...
                                if not np.isnan(s_a_val) and (s_a_val + s_b_val > 0): return f"{p_b_name} (완료)"
                                return p_b_name
                        ...
                        if p_a and p_b:
                            ...
                            if is_completed_ind: st.warning(f"⚠️ {p_a} vs {p_b} 경기는 이미 결과가 입력되었습니다.")
                            ...
                            with c5:
                                if st.button(" 저장", type="primary", use_container_width=True, key="btn_save_ind",
                                             disabled=is_completed_ind):
                ...
                # 관리자는 표에서 직접 점수 수정 가능, 일반 유저는 클릭해서 상세 전적만 확인 가능
                if is_admin:
